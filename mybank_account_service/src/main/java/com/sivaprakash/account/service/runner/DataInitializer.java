...
public class DataInitializer {
	...
	public CommandLineRunner insertDefaultAccounts(AccountRepository accountRepository) {
		return args -> {
			...
			Account account1 = new Account(null, "", 1, "ACC1234567890", new BigDecimal("1000.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.ACTIVE);

			Account account2 = new Account(null, "", 2, "ACC0987654321", new BigDecimal("2000.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.INACTIVE);

			Account account3 = new Account(null, "", 1, "ACC1122334455", new BigDecimal("500.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.FROZEN);

			Account account4 = new Account(null, "", 1, "ACC1234567891", new BigDecimal("100000.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.ACTIVE);

			Account account5 = new Account(null, "", 2, "ACC1234567892", new BigDecimal("200000.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.INACTIVE);

			Account account6 = new Account(null, "", 1, "ACC1234567893", new BigDecimal("500000.00"), "USD",
					LocalDateTime.now(), LocalDateTime.now(), AccountStatus.FROZEN);
